...
void calculate_integral_image(float *image, int width, int height, float *integral_image)
{
    ...
    for(i=1; i<height; ++i)
    {
        ...
        for(j=0; j<width; ++j)
        {
            // the sum of current  i st row
            rs += image[i*width+j];
            //Add the previous (i-1) row
...
//otsu
double cal_otsu_threshold(QImage &image)
{

    QImage grayImage = image.convertToFormat(QImage::Format_Grayscale8);
    int width = grayImage.width();
    int height = grayImage.height();
    int pixel_num = width*height;//像素总数


    const int gray_level = 256;
    int hist[gray_level] = {0};//灰度直方图，下标是灰度值，保存内容是灰度值对应的像素点总数
     uchar *bits = nullptr;
     bits = grayImage.bits();
    ...
    double Threshold=0; //Otsu算法阈值
    double Variance=0; //类间方差中间值保存
    double w_foreground=0; //前景像素点数所占比例
    double w_background=0; //背景像素点数所占比例
    double mean_foreground=0; //前景平均灰度
    double mean_background=0; //背景平均灰度
    ...
    for(int i=0;i<255;i++)
    {
        ...
        for(int j=0;j<255;j++) //背景部分各值计算
        {
            if(j <i)
            {
                 //***********背景各分量值计算**************************
                w_background+=hist[j];  //背景部分像素点总数
                mean_background+=j*hist[j]; //背景部分像素总灰度和
            }
            else
            {
                   //***********前景各分量值计算**************************
                w_foreground+=hist[j];  //前景部分像素点总数
                mean_foreground+=j*hist[j]; //前景部分像素总灰度和
            }

        }

        mean_background=mean_background/w_background; //背景像素平均灰度
        w_background=w_background/pixel_num; // 背景部分像素点数所占比例

        mean_foreground=mean_foreground/w_foreground; //前景像素平均灰度
        w_foreground=w_foreground/pixel_num; // 前景部分像素点数所占比例


        //***********类间方差计算******************************
        double mean_image =  mean_foreground *w_foreground+mean_background*w_background;
        double varValueI=w_foreground*(mean_foreground-mean_image)*(mean_foreground-mean_image) + w_background*(mean_background-mean_image)*(mean_background-mean_image) ;//当前类间方差计算
        if(Variance<varValueI)//找出最大的类间方差
        {
            Variance=varValueI;
            Threshold=i;
        }
...
//自动阈值分割
QImage autothreshSegmentation (QImage &src_image)
{

   double threshold = cal_otsu_threshold(src_image);
    //double threshold  = 127;
    int width = src_image.width();
    int height = src_image.height();

    QSize size = src_image.size();
